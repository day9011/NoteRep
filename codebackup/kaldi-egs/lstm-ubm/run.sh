...
# Train tri4, which is LDA + MLLT + SAT, on train data.
echo "tri4 : LDA + MLLT + SAT Training & Decoding"

steps/train_sat.sh --cmd "${train_cmd}" \
 ${numLeavesSAT} ${numGaussSAT} data/train_asr data/lang exp/tri3_ali exp/tri4
...
# Train tri5, which is LDA + MLLT + SAT, on train data.
steps/train_sat.sh --cmd "$train_cmd" \
 ${numLeavesSAT5} ${numGaussSAT5} data/train_asr data/lang exp/tri4_ali exp/tri5
...
steps/make_fbank.sh --nj ${num_job} \
...
steps/make_fbank.sh --nj ${num_job} \
		      --cmd "$train_cmd" data/test_hires exp/make_test_hires/test $mfccdir || exit 1;
...
for x in train_sup test_sup dev_sup;do
  cp -r data/${x} data/${x}_dnn
  steps/make_mfcc.sh --cmd "${train_cmd}" --nj ${num_job} --mfcc-config conf/mfcc_ubm.conf data/${x} exp/make_mfcc/${x} ${mfccdir}
  utils/fix_data_dir.sh data/${x}
  sid/compute_vad_decision.sh --nj ${num_job} --cmd "${train_cmd}" data/${x} exp/make_vad ${vaddir}
  utils/fix_data_dir.sh data/${x}
done

for x in train_sup_dnn test_sup_dnn dev_sup_dnn;do
  steps/make_fbank.sh --cmd "${train_cmd}" --nj ${num_job} data/${x} exp/make_mfcc/${x} ${mfccdir}
  utils/fix_data_dir.sh data/${x}
done
...
lstmdir=exp/nnet3/lstm_ld5/final.mdl

#init full_ubm from nnet3
local/init_full_ubm_from_lstm.sh --cmd "${train_cmd}" --use-gpu ${use_gpu} --nj ${num_job} \
	data/train_sup data/train_sup_dnn ${lstmdir} exp/full_ubm
...
sid/train_ivector_extractor.sh --cmd "${train_cmd}" \
	...
	exp/full_ubm/final.ubm data/train_sup exp/extractor_sup_gmm

#Train i-ivector extractor on DNN-UBM
local/train_ivector_extractor_lstm.sh --cmd "${train_cmd}" --use-gpu ${use_gpu} --nj 2 \
	--nnet-job-opt "--mem 15G" --min-post ${min_post} --ivector-dim ${ivector_dim} --num-iters ${num_iter} --num-processes 1 \
	exp/full_ubm/final.ubm ${lstmdir} data/train_sup data/train_sup_dnn exp/extractor_dnn
...
local/extract_ivectors_lstm.sh --cmd "${train_cmd}" --nj ${num_job} --use-gpu ${use_gpu} \
	exp/extractor_dnn ${lstmdir} data/train_sup data/train_sup_dnn exp/ivectors_sup_dnn_train
...
local/extract_ivectors_lstm.sh --cmd "${train_cmd}" --nj ${num_job} --use-gpu ${use_gpu} \
	exp/extractor_dnn ${lstmdir} data/test_sup data/test_sup_dnn exp/ivectors_sup_dnn_test

local/extract_ivectors_lstm.sh --cmd "${train_cmd}" --nj ${num_job} --use-gpu ${use_gpu} \
	exp/extractor_dnn ${lstmdir} data/dev_sup data/dev_sup_dnn exp/ivectors_sup_dnn_dev
