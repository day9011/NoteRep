...
int main(int argc, char *argv[])
{
	...
	const char *usage = "Acquire test result for one person's pcm.\n"
		"Usage: wav_ivector [options] <file.pcm> <ubm-file> <ie-file> gmm.ubm plda ivector_path valid_frames\n"
		"Options: --config\n"
	...
	std::string plda = po.GetArg(5);
	std::string ivectorpath= po.GetArg(6);
	std::string num_valid_frames_ = po.GetArg(7);
	...
	_file.close();
	InitSRE ubm;
	ubm.ReadUBMFile(final_ubm, final_ie, gmm_ubm, plda);
	CpuCompute comp;
