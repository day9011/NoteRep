...
#just support single enroll voice
class SreRegister(RequestHandler):
    ...
    def post(self):
        ...
        try:
            ...
            if len(userinfo) > 0:
                logger.warning('the user %s was enrolled, verification just use latest ivector' % (userid))
            sql_command = 'INSERT INTO user (userid, wavpath, ivecpath) values ("%s", "%s", "%s");' % (userid, upfilepath, ivecfile)
