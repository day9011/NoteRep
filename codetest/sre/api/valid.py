...
class SreValid(RequestHandler):
    ...
    def post(self):
        ...
        try:
            ...
            dependent_threshold = float(config_get('dependent-threshold', 'sre'))
            independent_threshold = float(config_get('independent-threshold', 'sre'))
            ...
            Args = [
                    ...
                    self_argument('TextRelevance', required=True, helpinfo='miss text relevance symbol')
                    ]
            ...
            if text_relevance == 'yes':
                ...
                logger.warning('text_relevance param maybe wrong:' + text_relevance)
            ...
            for meta in file_metas:
                ...
                upfilepath = wavpath.rstrip('/') + '/' + str(wav_uuid) + '_' + str(isself_symbol) + '_' + filename
            ...
            if score > threshold:
                ret['message'] += 'you are in person, threshold is ' + str(threshold)
            else:
                ret['message'] += 'you are not in person, threshold is ' + str(threshold)
