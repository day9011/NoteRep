...
void CuDevice::SelectGpuId(std::string use_gpu) {
  ...
  if (use_gpu != "yes" && use_gpu != "no" && use_gpu != "optional" && use_gpu != "wait") {
    KALDI_WARN << "Please choose : --use-gpu=yes|no|optional|wait, passed '" << use_gpu << "'";
  }

  // Make sure this function is not called twice!
  if (Enabled()) {
    KALDI_WARN << "There is already an active GPU " << active_gpu_id_
  ...
  if (IsComputeExclusive()) {
    ...
    if(SelectGpuIdAuto()) {
      ...
      if (use_gpu == "yes") {
        KALDI_WARN << "Error acquiring GPU.";
      }
      if (use_gpu == "optional") {
...
bool CuDevice::SelectGpuIdAuto() {
  ...
  do {
    ...
    // contains call to KALDI_WARN (which will cause the program to abort)
...
void* CuDevice::Malloc(size_t size) {
  ...
  if (e != cudaSuccess) {
    PrintMemoryUsage();
    KALDI_WARN << "CuDevice::Malloc: cannot allocate the requested memory"
      << " (" << size << " bytes )";
  }
  return ret_ptr;
}
...
bool CuDevice::SelectGpuIdManual(int32 gpu_id) {
	int32 n_gpu = 0;
	cudaError_t e = cudaGetDeviceCount(&n_gpu);
	if (n_gpu == 0) {
		KALDI_WARN << "No CUDA devices found";
		if (e != cudaSuccess) {
			KALDI_WARN << "cudaGetDeviceCount error";
			return false;
		}
		return false;
	}
	if (gpu_id > n_gpu) {
		KALDI_WARN << "choose gpu id error";
		return false;
	}
	...
	switch(ret) {
		case cudaSuccess: {
				...
				KALDI_LOG << "cudaSetDevice(" << gpu_id << "): "
						  << name << "\t" << mem_stats;
				if (total <= 0) {
					KALDI_LOG << "Total memory reported for device " << gpu_id << " is less";
					cudaDeviceReset();
					return false;
				}
				float mem_ratio = total > 0 ? free/(float)total : 0;
				KALDI_LOG << "Success selecting device" << gpu_id << "free mem ratio: " << mem_ratio;
			 }break;
		case cudaErrorDeviceAlreadyInUse:
				KALDI_WARN << "device" << gpu_id << " is already in use";
				return false;
		case cudaErrorInvalidDevice:
				KALDI_WARN << "device" << gpu_id << "is invalid";
				return false;
		default:
				KALDI_WARN << "cudaSetDevice(" << gpu_id << ") returned" 
						   << ret << ", " << cudaGetErrorString((cudaError_t)ret);
				return false;
	}
	KALDI_ASSERT(cudaSuccess == cudaDeviceSynchronize());
	int32 act_gpu_id;
	e = cudaGetDevice(&act_gpu_id);
	KALDI_ASSERT(act_gpu_id == gpu_id);
	active_gpu_id_ = act_gpu_id;
	CU_SAFE_CALL(cublasInit());
	return true;
}
